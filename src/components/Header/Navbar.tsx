...
const Navbar: React.FC<NavbarProps> = ({ onQuoteClick }) => {
    ...
    const links = [
        ...
        { href: '/regular', label: 'Regular Cleaning', action: null },
    ...
    const NavItem = ({ href, label, action }: { href: string; label: string; action: (() => void) | null }) => (
        <Link href={href}
            ...
            className="relative text-gray-700 font-medium text-base hover:text-[#0F5E46] transition-colors duration-300 group py-2">
            {label}
    ...
    const MobileNavItem = ({ label, action, href }: { label: string; action?: (() => void) | null; href?: string }) => {
        const commonClasses = "block text-md font-semibold text-gray-800 hover:text-[#0F5E46] py-3 border-b border-gray-100 transition duration-300 w-full text-left";
        if (href) {
            return (
                <Link
                    href={href}
